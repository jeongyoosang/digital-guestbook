  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-6">
      <div className="w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-xl overflow-hidden border border-border/60">
        <div className="p-4 sm:p-5 border-b border-border/60 bg-background/60">
        ...
        <div className="p-4 sm:p-5 border-t border-border bg-background/60">
...
export const ReservationForm = () => {
  ...
  // ✅ 상단 링크 동작(필요하면 라우트만 바꿔)
  const goHome = () => navigate("/");
  const goServiceFlow = () => navigate("/#service-flow"); // 너가 서비스흐름 섹션 만들면 이 id로 맞추면 됨
  const goMyReport = () => navigate("/login"); // 또는 /app 로 쓰는 구조면 "/app"
    ...
    return (
      ...
      >
        ...
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6">
          <div className="rounded-3xl bg-background/70 backdrop-blur-xl border border-border/60 shadow-[0_20px_60px_rgba(15,23,42,0.10)] p-8 sm:p-12 text-center">
            <h2 className="text-3xl sm:text-4xl font-semibold mb-4 text-foreground">
              감사합니다 💐
            </h2>
            <p className="text-base sm:text-lg text-muted-foreground leading-relaxed">
              문의가 정상 접수되었습니다.<br />
              예약 확정 안내와 디지털 방명록 링크는<br />
              <span className="text-foreground font-semibold">공식 카카오톡 채널</span>로 발송됩니다.
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <section id="reservation" className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-3xl px-4 sm:px-6 pt-10 sm:pt-14 pb-16 sm:pb-20">
        {/* ✅ Top bar */}
        <div className="flex items-center justify-between">
          <button
            type="button"
            onClick={goHome}
            className="inline-flex items-baseline gap-2 hover:opacity-90 transition"
          >
        ...
        <div className="mt-10 sm:mt-12 text-center">
          ...
          <p className="mt-3 text-sm sm:text-base text-muted-foreground leading-relaxed">
            날짜와 기본 정보만 먼저 받아요.<br className="hidden sm:block" />
            예식 설정은 예식 2주 전에 진행됩니다.
          </p>
        </div>

        {/* ✅ 안내 박스: “하얀 테두리” 느낌 제거 */}
        <div className="mt-6 rounded-2xl bg-background/55 backdrop-blur border border-border/0 shadow-[0_10px_30px_rgba(15,23,42,0.06)] px-5 py-4 text-sm text-muted-foreground">
          제출 후 카카오톡으로 입금 및 확정 안내를 드립니다.<br />
          입금 확인 후 예약이 확정됩니다.
        </div>

        <form
          onSubmit={handleSubmit(onSubmit)}
          className="mt-6 rounded-3xl bg-background/70 backdrop-blur-xl border border-border/60 shadow-[0_20px_60px_rgba(15,23,42,0.10)] p-5 sm:p-8 space-y-6"
        >
          {/* 이름 + 역할 + (기타시 관계) */}
          <div>
            ...
            <Input
              id="name"
              {...register("name")}
              className="mt-2 bg-background border-border focus-visible:ring-ring"
            />
            {errors.name && (
            ...
            <div className="flex gap-5 mt-3 text-sm">
              {["신랑", "신부", "기타"].map((r) => (
                <label key={r} className="flex items-center gap-2 text-foreground/80">
                  <input type="radio" value={r} {...register("role")} className="accent-[hsl(var(--ring))]" />
                  {r}
                </label>
              ))}
            </div>
                  ...
                  <Popover open={datePopoverOpen} onOpenChange={setDatePopoverOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className={cn(
                          "w-full justify-start text-left font-normal mt-2 border-border bg-background",
                ...
                <div className="mt-2 grid gap-2">
                  <div className="flex flex-col md:flex-row gap-2 md:items-start">
                    <Button
                      ...
                      className="w-full md:w-auto border-border bg-background hover:bg-muted/40 text-foreground"
                    >
                    ...
                    {venueName && (
                      <div className="w-full md:flex-1 md:min-w-0 rounded-xl border border-border/60 p-3 text-sm bg-background/60 overflow-hidden">
          ...
          {/* 동의 */}
          <div className="rounded-2xl bg-background/55 border border-border/60 p-4">
            <label className="flex items-start gap-3">
              <input type="checkbox" {...register("agree")} className="mt-1 h-4 w-4 accent-[hsl(var(--ring))]" />
